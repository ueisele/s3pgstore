...
| `S3PGSTORE_S3_MAX_RETRY_ATTEMPTS` | `AWS_MAX_ATTEMPTS` or `5` | SDK retry budget per logical S3 op (1 initial + retries). Equal-jitter backoff windows 100ms..10s. Honours the AWS-standard `AWS_MAX_ATTEMPTS` as fallback. |
