...
type Config[T any] struct {
	...
	// S3 wiring. All three required. The S3Client must be
	// pre-instrumented via s3client.WithDefaults — the library
	// no longer wraps the caller's client. This shifts the four
	// client-level tuning knobs (max-open-connections, retry
	// budget, rate-limit RPS, burst) onto s3client.Options at
	// the construction site, so multiple Stores sharing one
	// s3.Client also share one rate limiter, one adaptive
	// retry token bucket, and one connection pool.
	//
	// The S3* prefix marks these as the "S3 namespace" — every
	// S3-related field on Config starts with S3 so they cluster
	// alphabetically and mirror the env-var convention
	// (S3PGSTORE_S3_*).
	S3Bucket string
	S3Prefix string
	S3Client *s3.Client
